...
Reproducibility
---------------
Seeding alone is not sufficient. The reduction kernels used for gradient
accumulation on graphics hardware are non-deterministic by default, so two runs
with the same seed agree on the first epoch to three decimal places and then
diverge, which is enough to reorder the nearest-neighbour list. `set_seeds()`
therefore enables deterministic kernels and routes every latent draw through
explicitly seeded generators. Determinism costs roughly 10-30% training time; if
an operation without a deterministic kernel is encountered, set
TF_DISABLE_OP_DETERMINISM=1 and treat the run as one member of an ensemble.

Because a single training is one draw from the space of generators, `--runs N`
repeats the whole pipeline with independent seeds and writes ensemble summaries:
means and standard deviations for the optimum, and the number of runs in which
each phase enters the ten nearest neighbours. Per-run artifacts are archived with
a __seedNN suffix, and the run whose optimum lies closest to the ensemble mean is
copied back to the canonical filenames for use as the representative run.
