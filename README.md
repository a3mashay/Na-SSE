...
the default MCD subset holds only about half the rows and the scatter matrix is
numerically rank-deficient, so the distances are unstable and the flagged set is not
reproducible: repeated fits of the raw estimator agree on roughly a fifth of it. With the
shrinkage estimator above, repeats agree at a Jaccard of about 0.90.
...
the global screen flagged 39 compositions, of which 38 met the 80% consensus threshold and
are reported as global outliers; the local screen flagged a further 39.
...
**Preprocessing.** Na-containing rows are selected and represented by 27 descriptors.
Features are clipped with a 3×IQR rule to reduce the influence of extreme values, then
scaled to [−1, 1] to match the generator's tanh output. A 10% split is held out. The
...
`requirements.txt` gives the minimum versions each script needs. It is deliberately loose,
so a fresh install will resolve to current releases, and results may differ slightly from
the published run: library versions matter here. Moving scikit-learn from 1.4 to 1.6, for
instance, changes the RandomForest splits behind the descriptor-attribution analysis, and
UMAP and HDBSCAN outputs are version-sensitive even under a fixed seed.

Two files record the exact environment that produced the reported numbers:

| File | Contents |
| --- | --- |
| `environment.json` | Python version, platform, the versions of the eleven libraries that drive the results, and the six thread-limit environment variables that were set |
| `requirements_lock.txt` | Full `pip freeze` of that session, including transitive dependencies such as SciPy, joblib and llvmlite |

To reproduce the published numbers exactly rather than approximately, install from the lock
...
Note that thread counts affect the result as well as versions. The published run sets
`PYTHONHASHSEED`, `OMP_NUM_THREADS`, `MKL_NUM_THREADS`, `OPENBLAS_NUM_THREADS`,
`NUMBA_NUM_THREADS` and `NUMEXPR_NUM_THREADS` to `0`/`1` **before** NumPy or Numba is
imported; setting them afterwards has no effect. The values used are recorded in
`environment.json`.
...
should sit at or above about 0.90. A value far below that means the covariance is still
