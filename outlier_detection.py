...
Component 1 of the code released with:
    ...
    "Unsupervised Screening and Generative Exploration of Sodium Solid-State
     Electrolyte Chemical Space"
    A. Mashayekhi, S. Khazraei, J. Bekou

Two complementary outlier scores are computed on the final working dataset of
Na-containing compositions:
  ...
  Global rarity  Robust Mahalanobis distance in a principal-component space
                 ...
                 taken from the support of a Minimum Covariance Determinant
                 (MCD) fit with the support fraction fixed at 0.75,
                 re-estimated on that support with Ledoit-Wolf shrinkage, and
                 inverted after discarding eigen-directions below EIG_FLOOR of
                 the largest eigenvalue. The screen is repeated with
                 independent random states and compositions flagged in at
                 least CONSENSUS_FRAC of the repeats are reported.
  ...
  Local rarity   Local Outlier Factor (k = 35, Euclidean metric) applied to the
                 ...
                 map used for HDBSCAN clustering. This score is deterministic
                 for a given embedding and is computed once.
...
Why the covariance is not estimated with MinCovDet alone
--------------------------------------------------------
Fitting MCD directly to ~80 principal components yields a scatter matrix that
is numerically rank-deficient, because the default MCD subset holds only about
half of the rows. Inverting it with a pseudo-inverse amplifies eigenvalues of
order 1e-17 into squared distances of order 1e15, and the resulting ranking is
not reproducible between executions: repeated fits of the raw estimator agree
on roughly a fifth of the flagged set. Fixing the support fraction, shrinking
the covariance on the MCD support, and flooring the eigenvalues brings the
condition number down by many orders of magnitude and raises the agreement
between repeats to ~0.95 (Jaccard).
...
-------
  ...
  outlier_repeat_stability.csv         Per-repeat agreement diagnostics
...
def robust_cov_mahal(X_cov: np.ndarray,
    ...
    MCD supplies the robust location and the inlying subset; the covariance is
    re-estimated on that subset with Ledoit-Wolf shrinkage so that it is
    positive-definite, and the inverse discards eigen-directions below
    `eig_floor` times the largest eigenvalue. See the module docstring for why
    the raw MinCovDet covariance combined with a pseudo-inverse is unusable
    here.
    """
