...
def load_and_merge(features_csv: str, summary_csv: str) -> pd.DataFrame:
    ...
    if "membership_prob" in summ.columns:
        # kind="mergesort" keeps the sort stable: ties in membership_prob would
        # otherwise be broken arbitrarily by quicksort, so the retained row could
        # differ between runs on the same input.
        summ = (
            summ.sort_values(["formula_pretty", "membership_prob"],
...
def robust_cov_mahal(X_cov: np.ndarray,
    ...
    support fraction is rank-deficient at ~80 PCs, so the distances become
    unstable and the flagged set stops being reproducible between runs.
    """
