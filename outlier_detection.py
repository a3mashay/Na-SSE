...
# - LOF computed in PCA or 2D UMAP space (set LOF_SPACE; see README)
...
LOF_NEIGHBORS   = 35
LOF_SPACE       = "umap" # local-outlier space: "pca" (95%-variance PCA space, as in the
                         # manuscript methods) or "umap" (2D UMAP embedding z1,z2, the
                         # variant that produced the published Tables 1-4). See README.
...
# ----------------- LOF (local outliers): space chosen via LOF_SPACE -----------------
# "pca"  -> LOF in the 95%-variance PCA space (the same embedding used for the robust
#           Mahalanobis distance), so the global and local metrics share one space.
#           This is the variant described in the manuscript methods.
# "umap" -> LOF in the 2D UMAP embedding (z1, z2) carried over from the summary CSV,
#           i.e. the same 2D map used for the plots. This variant produced the
#           published Tables 1-4 and is the default.
if LOF_SPACE == "pca":
    X_lof = X_pc
elif LOF_SPACE == "umap":
    if not {UMAP_LABELS[0], UMAP_LABELS[1]}.issubset(df.columns):
        raise KeyError(
            f"UMAP coordinates {UMAP_LABELS} not in df; LOF_SPACE='umap' needs the 2D "
            "UMAP embedding (z1, z2) carried over from the summary CSV."
        )
    X_lof = df[[UMAP_LABELS[0], UMAP_LABELS[1]]].to_numpy(dtype=float)
else:
    raise ValueError(f"LOF_SPACE must be 'pca' or 'umap', got {LOF_SPACE!r}")

lof = LocalOutlierFactor(n_neighbors=LOF_NEIGHBORS, metric="euclidean", n_jobs=-1)
_ = lof.fit_predict(X_lof)
